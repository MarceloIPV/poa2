		...
		success:function(response){
		   	...
		   	for (x of informacionSeleccionada) {
				...
				if(idx != -1) {
					...
					$(".fila__ac"+x.idActividades).append('<td class="filaEliminar_'+x.idActividades+'"><center><button type="button" class="btn btn-danger boton_eliminar_act" id="eliminarAct'+x.idActividades+'" idActividad="'+x.idActividades+'" >Eliminar Actividad</button></center></td>');

					$.getScript("layout/scripts/js/incrementosDecrementos/metodos.js",function(){
						eliminarActividades($(".boton_eliminar_act"));
					});
					...
					if (idx2 != -1) {
						...
						$(".filaEliminar_"+x.idActividades).html(" ");
...
segmentosJsAjax($(".body__actividadesEs__incrementos__insertar"),"actividadesPoas__Incrementos__Creacion");
